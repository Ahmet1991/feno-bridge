...
if [ "$#" -lt 1 ]; then
  echo "Feno Bridge AppImage runner requires an AppImage path" >&2
  exit 64
fi
APPIMAGE_PATH="$1"
shift
case "$APPIMAGE_PATH" in
  /*) ;;
  *) echo "Feno Bridge AppImage path must be absolute" >&2; exit 64 ;;
esac
if [ ! -f "$APPIMAGE_PATH" ] || [ ! -x "$APPIMAGE_PATH" ]; then
  echo "Feno Bridge AppImage is unavailable: $APPIMAGE_PATH" >&2
  exit 66
fi
...
if [ ! -x "$APP_DIR/AppRun" ]; then
  echo "Feno Bridge AppImage fallback extraction produced no executable AppRun" >&2
  exit 70
fi
