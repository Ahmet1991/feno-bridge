...
test("launcher publishes native packages for all supported desktop operating systems", () => {
  ...
  assert.equal(manifest.build.productName, "Feno Bridge");
  assert.equal(manifest.build.artifactName, "feno-bridge-${version}-${os}-${arch}.${ext}");
...
test("release installers resolve checksummed native launcher assets", () => {
  ...
  assert.match(windowsInstaller, /feno-bridge-\$Version-win-\$Arch\.exe/);
