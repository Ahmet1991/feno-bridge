...
test("skill file experiment uses setup in production and DEV, and rejects manual mode", async () => {
  const production = hostFor({ mode: "full", browserInteractionMode: "automatic", appName: "Codex Native2" });
  ...
  assert.equal(production.invocation().args.includes("--app-name"), false);
  assert.equal((await production.host.setSkillAttachments(false)).enabled, false);
  assert.equal(production.invocation().args.includes("--inline-skills"), true);
  assert.equal(production.invocation().args.includes("--app-name"), false);
  const dev = devHostFor({ mode: "full", browserInteractionMode: "automatic", appName: "Codex Native2 DEV" });
  assert.equal((await dev.host.setSkillAttachments(true)).enabled, true);
  assert.equal(dev.invocation().args.includes("--skill-attachments"), true);
  ...
  assert.equal(dev.invocation().args.includes("--app-name"), false);
  const manual = hostFor({ mode: "full", browserInteractionMode: "manual", appName: "Codex Native2" }, "manual");
