...
const en = {
  ...
  biggerContextBody: "Warning: keeps small turns as one message, then splits large context across 2–12 messages (up to 80,000 characters each). The final part starts the task without an extra request. Model context and compaction thresholds increase by up to three times; restart Codex after changing it. Extra requests may increase rate limits or temporary cooldowns. Disabled by default.",
  skillAttachments: "Skills as files (experimental)",
  skillAttachmentsBody: "Upload explicitly selected Codex skills as text files. When a long task fits one model message but exceeds the browser text limit, its complete context can also use one text file instead of multiple messages. Off by default; shares the attachment limit with images.",
  ...
  continue: "Continue",
  supportTitle: "About Feno Bridge",
  supportBody: "Feno Bridge is shared through a private GitHub repository. These links are optional.",
  star: "Open Feno Bridge repository",
  starred: "Repository opened",
  follow: "Open upstream author on X",
  followed: "Upstream author opened",
...
const zh: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：较小回合仍作为一条消息发送；较大的上下文会拆分为 2–12 条消息（每条最多 80,000 字符），最后一部分会直接开始任务，不会额外发送请求。模型上下文和压缩阈值最多扩大三倍；更改后请重启 Codex。额外请求可能提高触发速率限制或临时冷却的概率。默认关闭。",
  skillAttachments: "技能作为文件（实验性）",
  skillAttachmentsBody: "将明确选择的 Codex 技能作为文本文件上传。长任务若符合单条模型消息的容量，但超过浏览器文本限制，也可将完整上下文作为一个文本文件上传，避免拆成多条消息。默认关闭；与图片共用附件数量限制。",
  ...
  continue: "继续",
  supportTitle: "关于 Feno Bridge",
  supportBody: "Feno Bridge 通过私有 GitHub 仓库共享。这些链接是可选的。",
  star: "打开 Feno Bridge 仓库",
  starred: "已打开仓库",
  follow: "在 X 上打开上游作者主页",
  followed: "已打开上游作者主页",
  finishWelcome: "打开启动器",
  setup: "设置",
  activity: "活动",
...
const ja: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：小さなターンは 1 件のメッセージとして維持し、大きなコンテキストは 2～12 件（1 件あたり最大 80,000 文字）に分割します。最後の部分から追加リクエストなしでタスクを開始します。モデルのコンテキスト上限とコンパクションしきい値は最大 3 倍になります。変更後は Codex を再起動してください。追加リクエストにより、レート制限や一時的なクールダウンが発生しやすくなる場合があります。初期設定では無効です。",
  skillAttachments: "スキルをファイルで送信（試験的）",
  skillAttachmentsBody: "明示的に選択した Codex スキルをテキストファイルとしてアップロードします。長いタスクがモデルの1メッセージには収まるもののブラウザーの文字数制限を超える場合、完全なコンテキストも1つのテキストファイルで送信できます。初期設定はオフで、画像と添付数の上限を共有します。",
  ...
  continue: "続ける",
  supportTitle: "Feno Bridge について",
  supportBody: "Feno Bridge は非公開の GitHub リポジトリで共有されています。これらのリンクは任意です。",
  star: "Feno Bridge リポジトリを開く",
  starred: "リポジトリを開きました",
  follow: "X でアップストリーム作者を開く",
  followed: "アップストリーム作者を開きました",
  ...
  setupTitle: "Feno Bridge をセットアップ",
...
const ko: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "경고: 작은 턴은 하나의 메시지로 유지하고, 큰 컨텍스트는 2–12개 메시지(메시지당 최대 80,000자)로 나눕니다. 마지막 부분에서 별도의 추가 요청 없이 작업을 시작합니다. 모델 컨텍스트와 컴팩션 임계값은 최대 3배까지 증가합니다. 변경 후 Codex를 다시 시작하세요. 추가 요청으로 인해 사용량 제한이나 일시적인 쿨다운이 더 자주 발생할 수 있습니다. 기본적으로 비활성화되어 있습니다.",
  skillAttachments: "스킬을 파일로 전송 (실험적)",
  skillAttachmentsBody: "명시적으로 선택한 Codex 스킬을 텍스트 파일로 업로드합니다. 긴 작업이 모델의 한 메시지에는 들어가지만 브라우저 글자 수 제한을 넘으면 전체 문맥도 텍스트 파일 하나로 보낼 수 있습니다. 기본값은 꺼져 있으며 이미지와 첨부 파일 수 제한을 공유합니다.",
  ...
  continue: "계속",
  supportTitle: "Feno Bridge 정보",
  supportBody: "Feno Bridge는 비공개 GitHub 저장소를 통해 공유됩니다. 이 링크들은 선택 사항입니다.",
  star: "Feno Bridge 저장소 열기",
  starred: "저장소 열림",
  follow: "X에서 업스트림 작성자 열기",
  followed: "업스트림 작성자 열림",
  finishWelcome: "런처 열기",
  setup: "초기 설정",
  activity: "활동",
  settings: "설정",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：小型回合仍以一則訊息傳送；大型上下文會拆成 2–12 則訊息（每則最多 80,000 字元）。最後一部分會直接開始任務，不會額外傳送請求。模型上下文與壓縮門檻最多提高至三倍；變更後請重新啟動 Codex。額外請求可能更容易觸發速率限制或暫時冷卻。預設關閉。",
  skillAttachments: "技能作為檔案（實驗性）",
  skillAttachmentsBody: "將明確選取的 Codex 技能作為文字檔上傳。若長任務可放進單則模型訊息，但超過瀏覽器文字限制，也可將完整脈絡作為一個文字檔上傳，避免拆成多則訊息。預設關閉；與圖片共用附件數量限制。",
  ...
  continue: "繼續",
  supportTitle: "關於 Feno Bridge",
  supportBody: "Feno Bridge 透過私人 GitHub 儲存庫分享。這些連結皆為選用。",
  star: "開啟 Feno Bridge 儲存庫",
  starred: "已開啟儲存庫",
  follow: "在 X 開啟上游作者",
  followed: "已開啟上游作者",
  finishWelcome: "開啟啟動器",
...
const tr: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "Uyarı: küçük turları tek mesajda tutar, büyük bağlamı mesaj başına en fazla 80.000 karakter olacak şekilde 2–12 mesaja böler. Son bölüm ek istek olmadan görevi başlatır. Model bağlamı ve sıkıştırma eşikleri en fazla üç katına çıkar; değiştirdikten sonra Codex'i yeniden başlatın. Ek istekler kullanım limitlerini veya geçici bekleme sürelerini artırabilir. Varsayılan olarak kapalıdır.",
  skillAttachments: "Skill'leri dosya olarak gönder (deneysel)",
  skillAttachmentsBody: "Seçilen Codex skill'lerini metin dosyası olarak yükler. Uzun bir görev modelin tek mesaj sınırına sığıyor ama tarayıcının metin sınırını aşıyorsa, eksiksiz bağlamı da bir metin dosyasıyla gönderebilir. Varsayılan kapalıdır; görsellerle aynı ek sınırını paylaşır.",
  manualSkillAttachmentsUnavailable: "Skill dosyası yükleme otomatik tarayıcı etkileşimi gerektirir.",
  ...
  continue: "Devam",
  supportTitle: "Feno Bridge hakkında",
  supportBody: "Feno Bridge özel bir GitHub deposuyla paylaşılır. Bu bağlantılar isteğe bağlıdır.",
  star: "Feno Bridge deposunu aç",
  starred: "Depo açıldı",
  follow: "Temel projenin geliştiricisini X'te aç",
  followed: "Geliştirici sayfası açıldı",
  ...
  checkingChatGptConnector: "ChatGPT connector'ü kontrol ediliyor",
  checkingLocalRuntime: "Yerel çalışma zamanı kontrol ediliyor",
  doctorConfigValid: "Yapılandırma geçerli ({path})",
  doctorBrowserReady: "Gömülü tarayıcıda oturum açık ve erişilebilir (pid {pid})",
  doctorManualBrowserReady: "Zero Risk gömülü tarayıcısına erişilebiliyor (pid {pid})",
  doctorCodexInstalled: "Codex yerel model rotası kurulu",
  doctorRuntimeOwned: "Arka plan çalışma zamanını Feno Bridge yönetiyor",
  doctorChromeFound: "Chrome çalıştırılabilir dosyası bulundu: {path}",
  doctorLoginVerified: "ChatGPT oturum durumu tarayıcı kanıtıyla doğrulandı",
  doctorMacServiceLoaded: "macOS arka plan servisi yüklü",
  doctorMacTunnelRunning: "macOS tünel servisi kurulu, yüklü ve çalışıyor",
  doctorProxyHealthy: "Responses proxy {endpoint} üzerinde sağlıklı",
  doctorTunnelBinaryInstalled: "Sabitlenmiş openai/tunnel-client ikilisi kurulu",
  doctorTunnelKeyStored: "Tünel çalışma zamanı anahtarı özel olarak saklanıyor",
  doctorTunnelRuntimeOwned: "Tünel çalışma zamanını Feno Bridge yönetiyor",
  doctorTunnelRuntimeReady: "Tünel çalışma zamanı sağlıklı ve hazır",
  doctorConnectorAvailable: "ChatGPT connector'ü {name} kullanılabilir",
};
