          ...
          {isLanguage ? (
            <div className="welcome-options" role="radiogroup" aria-label={localized.chooseLanguage}>
              ...
              <WelcomeOption
                ...
                detail={localized.turkish}
                label={localized.turkish}
                marker="TR"
...
function LauncherShell({
  ...
  const updateVisible = snapshot.update.status !== "disabled";
    ...
    <ContentSurface narrow title={devProfile ? copy.devSettingsTitle : copy.settingsTitle}>
      ...
      <div className="settings-list">
        ...
        <SettingRow body={snapshot.state.browserInteractionMode === "manual"
          ...
          : copy.skillAttachmentsBody} label={copy.skillAttachments}>
          <Switch
  ...
  return (
    <span className={`brand-mark${small ? " is-small" : ""}`}>
      <svg aria-hidden="true" viewBox="0 0 24 24">
        <path d="M5.5 20V4.5H19M5.5 11.5H16" fill="none" stroke="currentColor" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M10 18c2.6-2.5 5.7-2.5 8.5 0" fill="none" stroke="#4BDBC9" strokeWidth="2.1" strokeLinecap="round" />
      </svg>
    </span>
  );
}
...
function formatTime(value: string, language: Language): string {
  ...
  return Number.isNaN(date.getTime())
    ? value
    : date.toLocaleTimeString(language === "tr" ? "tr-TR" : "en", {
        hour: "2-digit",
        minute: "2-digit",
        second: "2-digit",
      });
