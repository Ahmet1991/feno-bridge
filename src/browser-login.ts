...
export async function captureSystemBrowserLogin(
  ...
  process.stdout.write(
    "Sign in with your passkey in the dedicated Chrome window. When Temporary Chat is ready, return to Feno Bridge and choose Continue.\n",
  );
...
export async function loginToChatGpt(
  ...
  try {
    ...
    const composer = page.getByRole("textbox", { name: "Chat with ChatGPT" }).or(
      // The shared contract, so a composer rename never reports itself as an expired login.
