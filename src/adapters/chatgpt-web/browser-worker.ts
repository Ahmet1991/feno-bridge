 ...
 */
export const CHATGPT_PERSONALIZED_NAME_PATTERN = /^(?:Personalized|个性化|Kişiselleştirilmiş)$/;
export const CHATGPT_UNPERSONALIZED_NAME_PATTERN = /^(?:Unpersonalized|非个性化|Kişiselleştirilmemiş)$/;
/** Anchored at the start only: the choice row may append its own description after the label. */
const CHATGPT_PERSONALIZED_CHOICE_PATTERN = /^(?:Personalized|个性化|Kişiselleştirilmiş)/;
...
export async function throwIfChatGptSessionFailureAlert(page: Page): Promise<void> {
  if (await chatGptExpiredSessionAlert(page).isVisible().catch(() => false)) {
    throw new ChatGptWebAdapterError(
      "The ChatGPT session has expired. Sign in again in Feno Bridge.",
...
export class ChatGptSubmissionRejectionObserver {
  ...
  private readonly onResponse = (response: Response): void => {
    ...
    this.checks.push(withChatGptBrowserObservationTimeout(response.json(), 3_000)
      .then(body => body?.detail?.code === "browser_message_too_large"
        ? new ChatGptWebAdapterError(
          "ChatGPT rejected this message because it exceeds the browser page-capacity limit. Compact the task before retrying.",
          { status: 413, errorType: "invalid_request_error", code: "browser_message_too_large", retryable: false },
...
export function assertChatGptWebMultipartInputWithinLimits(
  ...
  // Transport parts distribute one model context; additional parts cannot enlarge that window.
          ...
          return {
            ...
            turns: {
              ...
              assistant: assistantTurns.map(element => ({
                ...
                completionActionCount: (element.closest("[data-turn-key]") ?? element)
                  .querySelectorAll(completionActionSelector).length,
                renderedCompletionActionCount: [...(element.closest("[data-turn-key]") ?? element)
                  .querySelectorAll(completionActionSelector)].filter(rendered).length,
              })),
            },
          };
        }, {
...
export class ChatGptBrowserWorker {
  ...
  private async runStage<T>(
    ...
    try {
      ...
      const timings = this.reactionTimings?.get(traceId);
      if (timings) timings.stages.set(stage, (timings.stages.get(stage) ?? 0) + durationMs);
      console.info(`[chatgpt-web] browser turn ${traceId} stage=${stage} completed durationMs=${Math.round(durationMs)}`);
      return value;
    } catch (error) {
  ...
  private async selectModelAndEffort(
    ...
    await settleChatGptUi();
    await throwIfChatGptRateLimitDialog(page);
    // The closed trigger already names the active mode; unreadable/localized-unknown labels
    // still use the existing slider and its post-selection verification.
    if (await currentEffort.getAttribute("aria-expanded").catch(() => null) === "false") {
      const closedLabel = await currentEffort.innerText().catch(() => "");
      if (selectedEffortMatches(closedLabel, uiEffortIndex)) {
        return {
          ...mode,
          selection: { url: page.url(), label: closedLabel.trim() },
        };
      }
    }
    ...
    // Per-tick locks are the authority when this build exposes them; otherwise the ARIA
    // range alone still proves which items exist. Never infer a lock that was not read.
    const readAvailableEffort = async (container: Locator, menu: Locator, slider: Locator) => {
      const snapshot = await readChatGptEffortSnapshot(container).catch(() => undefined);
      const state = snapshot ?? parseChatGptEffortSliderState(
      ...
      if (snapshot && !snapshot.available[uiEffortIndex]) {
        throw new ChatGptWebAdapterError(
          `ChatGPT locks the browser option requested for item index ${uiEffortIndex} behind an upgrade. `
    ...
    let sliderState = await readAvailableEffort(sliderContainer, activation.menu, effortSlider);
    ...
    while (sliderState.value !== targetValue) {
      ...
      do {
        sliderState = await readAvailableEffort(sliderContainer, activation.menu, effortSlider);
    ...
    await settleChatGptUi();
    const selectedState = await readAvailableEffort(sliderContainer, activation.menu, effortSlider);
    ...
    const confirmedState = await readAvailableEffort(
      confirmation.sliderContainer, confirmation.menu, confirmation.slider,
    );
  ...
  private async selectConnector(
    ...
    // A visible, exact chip is stronger evidence than opening the mention menu again.
    ...
    const ensurePersonalizationWithRecovery = async (): Promise<void> => {
      try {
        ...
        try {
          ...
          await runPersonalizationPreflight();
          sessionKey = await this.personalizationSessionKey?.(page);
        } catch (retryError) {
          throwIfPromptAttachmentAborted(abortSignal);
          throw chatGptConnectorUnavailableError(
    ...
    if (!cachedProof && !alreadyProved) await ensurePersonalizationWithRecovery();
    try {
      ...
      while (attemptBudget.triggerAttempts < MAX_CHATGPT_CONNECTOR_TRIGGER_ATTEMPTS) {
        ...
        try {
          ...
          if (cachedProof && !visibleRows.includes(this.config.appName)) {
            ...
            attemptBudget.triggerAttempts = 0;
            continue;
          }
  ...
  private async responseDomSnapshot(
    ...
    const observed = await responseTurn.evaluate((element, options) => {
      ...
      // [data-turn-key] group; watch and search that exchange, not only the answer body.
      const exchange = root.closest<HTMLElement>("[data-turn-key]") ?? root;
      type ObserverState = {
        id: number;
        revision: number;
      ...
      if (!observerState) {
        ...
        state.observer.observe(exchange, {
          subtree: true,
          childList: true,
          characterData: true,
          attributes: true,
      ...
      const completionAction = rendered
        ? [...exchange.querySelectorAll<HTMLElement>(options.completionActionSelector)]
          .filter(renderedInDom)
  ...
  private async runBrowserTurn(
    ...
    const reactionStartedAt = performance.now();
    if (turn.abortSignal?.aborted) throw new DOMException("ChatGPT web turn aborted", "AbortError");
    ...
    try {
      ...
      const reaction = this.reactionTimings?.get(turn.traceId);
      if (reaction) console.info(formatBrowserTurnReactionLog({
        traceId: turn.traceId,
        elapsedMs: performance.now() - reaction.startedAt,
        reused: reuseConversation,
        ...(prepared.multipart ? { multipartParts: prepared.multipart.parts.length } : {}),
