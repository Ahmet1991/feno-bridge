 ...
 * compaction starts at three parts and can grow up to the browser transport maximum.
 */
export function resolveBiggerContextMultipartParts(
  ...
  const compile = (parts?: ChatGptWebMultipartPartCount): CompiledChatGptWebPrompt => compileChatGptWebPrompt(
    ...
    { experimentalMultipartParts: parts, experimentalSkillAttachments, multipartRecordWeightCache },
  );
  ...
  for (
    let parts = initialParts ?? 2;
    parts <= CHATGPT_WEB_MULTIPART_MAX_PARTS;
    parts += 1
  ) {
    if (fits(compile(parts))) return parts;
  }
  return CHATGPT_WEB_MULTIPART_MAX_PARTS;
}
