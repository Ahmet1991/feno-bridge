...
async function loginCommand(args: string[]): Promise<void> {
  ...
  if (!launcherControl) {
    ...
    if (config.browserHost === "launcher") {
      throw new Error("ChatGPT login is owned by the launcher; open Feno Bridge and use its Sign in step");
    }
...
async function setupCommand(args: string[]): Promise<void> {
  ...
  if (result.capabilityRefreshWarning) stderr.write(`Warning: ${result.capabilityRefreshWarning}\n`);
...
async function uninstallCommand(args: string[]): Promise<void> {
  ...
  if (config?.browserHost === "launcher" && !launcherControl) {
    throw new Error(
      "Launcher-owned integration must be removed from Feno Bridge Settings so the active runtime can be drained safely.",
    );
  }
