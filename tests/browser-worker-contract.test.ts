...
test("assistant tracking accepts a replacement turn during proven MCP continuation", async () => {
  ...
  const rebound = await worker.reconcileAssistantTurnBinding(
    ...
    },
    {
      ...
      acceptedTurnIdentities: ["conversation-turn-user-1", "conversation-turn-assistant-1"],
    },
    undefined,
    true,
  );
...
test("only a size rejection of the current owned browser submission is non-retryable", async () => {
  ...
  respond(current);
  expect(await observer.failure()).toBeUndefined();
  const ownedCapacity = makeRequest(); page.emit("request", ownedCapacity);
  respond(ownedCapacity, "browser_message_too_large");
  expect(await observer.failure()).toMatchObject({
    status: 413, code: "browser_message_too_large", errorType: "invalid_request_error", retryable: false,
  });
  observer.dispose();
