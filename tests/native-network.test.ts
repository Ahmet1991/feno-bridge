...
test("native proxy selection preserves Chromium's first route and rejects protocol guessing", () => {
  ...
  for (const value of [
    "",
    undefined,
    "SOCKS5 localhost:1080; PROXY localhost:8080",
    "PROXY user:secret@host:8",
    "PROXY host:8#secret",
...
test("native fetch reaches a proxy-only target, refreshes routing, and never retries a failed route", async () => {
  ...
  writeFileSync(descriptor, JSON.stringify({
    ...
    control: { endpoint: control.url.origin, token },
  ...
  const request = () => new Request("http://native-proxy-regression.invalid/responses", {
    method: "POST",
    body: "native request",
    headers: { authorization: "Bearer codex-test-only" },
  ...
  try {
    ...
    expect(calls).toEqual([{
      url: request().url,
      authorization: "Bearer codex-test-only",
      body: "native request",
    }]);
    route = "DIRECT";
